...
def main():
    ...
    os.mkdir(folder_path + "GiD/")
    os.mkdir(folder_path + "CAD/")
    os.mkdir(folder_path + "LsDyna/")
    ...
    embedding_flag = True
    embedder = TrIGA_Application.TrIGA(filename, lower_point, upper_point, number_of_elements, order,
    ...
    number_trimmed_elements = 0
    number_untrimmed_elements = 0
    points_inside_vtk = []
    points_reduced_vtk = []

    # Setup knotspan writer
    number_boundary_triangles = 0
    for element in elements:
        if element.IsTrimmed():
            number_trimmed_elements += 1
            #print(element.GetNumberBoundaryTriangles())
            number_boundary_triangles += element.GetNumberBoundaryTriangles()
            for point_trimmed_reduced in element.GetIntegrationPointsTrimmed():
                ...
                if(weight  > 0.0 ):
                    ...
                    points_reduced_vtk.append(point_trimmed_reduced)
            ...
            number_untrimmed_elements += 1
            for point_inside in element.GetIntegrationPointsInside():
                points_inside_vtk.append(point_inside)
                points_all.append(point_inside)

    surface_mesh_file = "./data/cylinder"
