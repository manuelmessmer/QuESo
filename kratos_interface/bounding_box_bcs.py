...
class BoundingBox():
    ...
    def __init__(self, lower_point: List[float], upper_point: List[float]) -> None:
        ...
        Args:
            lower_point (List[float]): The lower corner of the bounding box (e.g., [x_min, y_min, z_min]).
            upper_point (List[float]): The upper corner of the bounding box (e.g., [x_max, y_max, z_max]).
        """
    ...
    def _is_inside(self, point: List[float]) -> bool:
        """
        Checks if a given point lies within the defined bounding box.

        Args:
            point (List[float]): A 3D point specified as a list [x, y, z].
...
class DirichletCondition(BoundingBox):
    ...
    This class overrides the apply method from the BoundingBox class to
    ...
    def __init__(self, lower_point: List[float], upper_point: List[float], disp: List[bool]) -> None:
        ...
        Args:
            lower_point (List[float]): The lower corner of the bounding box (e.g., [x_min, y_min, z_min]).
            upper_point (List[float]): The upper corner of the bounding box (e.g., [x_max, y_max, z_max]).
            disp (List[bool]): A vector indicating the displacement conditions for each axis.
...
class NeumannCondition(BoundingBox):
    ...
    This class overrides the `apply` method from the `BoundingBox` class to apply a force
    ...
    def __init__(self, lower_point: List[float], upper_point: List[float], force: List[float] ) -> None:
        ...
        Args:
            lower_point (List[float]): The lower corner of the bounding box (e.g., [x_min, y_min, z_min]).
            upper_point (List[float]): The upper corner of the bounding box (e.g., [x_max, y_max, z_max]).
            force (List[float]): The total force to be applied in the X, Y, and Z directions.
                                  (e.g., [fx, fy, fz]).
        """
    ...
    def apply(self, model_part: KM.ModelPart) -> None:
        ...
        node_id_force = [node.Id for node in model_part.Nodes if self._is_inside([node.X0, node.Y0, node.Z0])]

        if node_id_force:
            num_nodes = len(node_id_force)
            ...
            for i, node_id in enumerate(node_id_force):
