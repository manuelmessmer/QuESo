...
BOOST_AUTO_TEST_CASE( Thingi10KFileTester ) {
    ...
    TIBRA_ERROR_IF("Thingi10KFileTester", argc_ != 4 ) << "Please provide following arguments: -- Filename n_min n_max\n";

    const std::string filename = argv_[1];

    const IndexType n_min = static_cast<IndexType>(std::stoi(argv_[2]));
    const IndexType n_max = static_cast<IndexType>(std::stoi(argv_[3]));
