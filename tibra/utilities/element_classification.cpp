...
bool ElementClassification::IsInside(const PointType& rPoint){


    //AABB_tree tree(rTriangleMesh);
    //tree.rebuild();

    //gen.seed(0);
    // for (uint32_t i = 0; i < 16; ++i) {
//std::cout << "Random: " << drandon(gen) << std::endl;
    //TriangleMesh::Vector3d randDir{2 * dis(gen) - 1, 2 * dis(gen) - 1, 2 * dis(gen) - 1};
    //     randDir.normalize();
    //     Ray ray(Vec3f(0), randDir);

    //gen.seed(0);
    int p_count = 0;
    double tree_timer = 0.0;
    double ray_timer = 0.0;
            ...
            for( auto r : result){
                ...
                if( ray.rayTriangleIntersect(p1, p2, p3, t, u, v) ) {
                    ...
                    if( u < 0.0+1e-14 || v < 0.0+1e-14 || sum_u_v > 1.0-1e-14 ){
                        std::cout << rPoint[0] << ", " << rPoint[1] << ", " << rPoint[2] << std::endl;
                        is_on_boundary = true;
                        break;
                    }
                }
