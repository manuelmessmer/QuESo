...
void MomentFitting::ComputeReducedPointsSurfaceIntegral(Element& rElement, const Parameters& rParam){
    ...
    while( residual > rParam.MomentFittingResidual() && iteration < max_iteration){
        ...
        residual = ComputeReducedPointsSurfaceIntegral(rElement, point_distribution_factor, rParam);
    ...
    if( residual > rParam.MomentFittingResidual() ){
...
double MomentFitting::ComputeReducedPointsSurfaceIntegral(Element& rElement, const int PointDistributionFactor, const Parameters& rParam) {
    ...
    if( !rParam.UseCustomizedTrimmedPositions() ){
        DistributeIntegrationPoints( rElement, new_integration_points, PointDistributionFactor, rParam);
    ...
    VectorType constant_terms(number_of_functions, 0.0);
    ...
    for( int i_x = 0; i_x <= rParam.Order()[0]*ffactor; ++i_x){
        for( int i_y = 0; i_y <= rParam.Order()[1]*ffactor; ++i_y ){
            for( int i_z = 0; i_z <= rParam.Order()[2]*ffactor; ++i_z){
                ...
                for( auto& triangle : rElement.GetTriangles() ){
                    ...
                    for( auto global_point : (*p_global_integration_points)){
                        ...
                        value[2] = f_x(local_point[0], i_x, a[0], b[0])*f_x(local_point[1], i_y, a[1], b[1])*f_x_integral(local_point[2], i_z, a[2], b[2]);
                        //value[0] = f_x_integral(local_point[0], i_x)*f_x(local_point[1], i_y)*f_x(local_point[2], i_z);
                        //value[1] = f_x(local_point[0], i_x)*f_x_integral(local_point[1], i_y)*f_x(local_point[2], i_z);
                        //value[2] = f_x(local_point[0], i_x)*f_x(local_point[1], i_y)*f_x_integral(local_point[2], i_z);
    ...
    while( change || (global_residual < allowed_residual && number_iterations < maximum_iteration) ){
        ...
        global_residual = NNLS::nnls(fitting_matrix, constant_terms, weights)/number_of_functions;
        rElement.AddResidual(global_residual);
        rElement.AddNumberIps(number_reduced_points);
...
double MomentFitting::f_x_integral(double x, int order, double a, double b){
    ...
    throw  std::runtime_error("MomentFitting :: Order out of range!\n");
}
