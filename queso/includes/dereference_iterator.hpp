...
public:
    ...
    explicit constexpr DereferenceIterator(const BaseIterator &rOther) : BaseIterator(rOther) {}
...
[[nodiscard]] constexpr DereferenceIterator<BaseIterator> operator+(
    ...
    return DereferenceIterator<BaseIterator>(static_cast<const BaseIterator&>(it) + n);
}

template <class BaseIterator>
[[nodiscard]] constexpr DereferenceIterator<BaseIterator> operator+(
        ptrdiff_t n, const DereferenceIterator<BaseIterator>& it) noexcept {
    return DereferenceIterator<BaseIterator>(n + static_cast<const BaseIterator&>(it));
}
