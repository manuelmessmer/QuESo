...
namespace factories {
    ...
    inline Unique<MainDictionaryType> CreateSettings()
    {
        ...
        auto p_background_grid_settings =
            MakeUnique<DictionaryType>(DictionaryType::KeySetInfosTypeTag<
        ...
        /*--- Background grid settings ---*/
        auto p_trimmed_quadrature_rule_settings =
            MakeUnique<DictionaryType>(DictionaryType::KeySetInfosTypeTag<
        ...
        p_trimmed_quadrature_rule_settings->SetValue(
            TrimmedQuadratureRuleSettings::neglect_elements_if_stl_is_flawed, true
        );

        // Add to Main.
        p_settings->SetSubDictionary(
            MainSettings::trimmed_quadrature_rule_settings, std::move(p_trimmed_quadrature_rule_settings)
        );

        /*--- Non background grid settings ---*/
        auto p_non_trimmed_quadrature_rule_settings =
            MakeUnique<DictionaryType>(DictionaryType::KeySetInfosTypeTag<
        ...
        // Set Defaults
        p_non_trimmed_quadrature_rule_settings->SetValue(
            NonTrimmedQuadratureRuleSettings::integration_method, IntegrationMethod::gauss
        );

        // Add to Main.
        p_settings->SetSubDictionary(
            MainSettings::non_trimmed_quadrature_rule_settings, std::move(p_non_trimmed_quadrature_rule_settings)
        );
    ...
    inline Unique<MainDictionaryType> CreateComponentInfo()
    {
        ...
        p_elapsed_time_info->SetSubDictionary(
            ElapsedTimeInfo::write_files_time_info, std::move(p_write_files_time_info)
        );
...
private:
    ...
    inline static const std::unordered_map<std::string, CreatorFunc> mRegistry =
        factories::detail::RegisterAll<TKeySetValuesTypeTag>();
    ///@}
