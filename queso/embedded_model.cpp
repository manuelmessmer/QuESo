...
void EmbeddedModel::CheckIfMeshIsWithinBoundingBox(const TriangleMeshInterface& rTriangleMesh) const {
    ...
    if( mSettings[MainSettings::general_settings].GetValue<IndexType>(GeneralSettings::echo_level) > 0 ){
        // Get grid dimensions.
        PointType lower_bound = mSettings[MainSettings::background_grid_settings].
            GetValue<PointType>(BackgroundGridSettings::lower_bound_xyz);
        PointType upper_bound = mSettings[MainSettings::background_grid_settings].
