...
namespace MeshUtilities {
    namespace {
        ...
        constexpr double MinimumQualityNormalizationDenominator = 1e-14;

    }  // namespace

    void Refine(TriangleMesh& rTriangleMesh, IndexType MinNumberOfTriangles, double ZeroArea)
    {
        while (rTriangleMesh.NumOfTriangles() < MinNumberOfTriangles) {
            ...
            for (const auto& v : old_vertices) { refined.AddVertex(v); }
    ...
    void Append(TriangleMesh& rTriangleMesh, const TriangleMesh& rNewMesh)
    {
        ...
        for (const auto& vertex : rNewMesh.Vertices()) { rTriangleMesh.AddVertex(vertex); }
        ...
        for (IndexType tid = 0; tid < source_triangles.size(); ++tid) {
            ...
            const Vector3i offset_triangle{ tri[0] + initial_vertex_count,
...
#pragma omp parallel for reduction(+ : area)
    ...
    double Volume(const TriangleMeshView& rTriangleMeshView)
    {
        ...
        rTriangleMeshView.VisitEachTriangle<WithNormals>([&](const auto& triangle) {
            ...
            const double contribution = Math::Dot(triangle.Normal, center) * TriangleUtilities::Area(triangle);
            const double updated = volume + contribution;
            if (std::abs(volume) >= std::abs(contribution)) {
                correction += (volume - updated) + contribution;
            } else {
                correction += (contribution - updated) + volume;
            }
            volume = updated;
        });
        return std::abs((volume + correction) / 3.0);
    }
...
#pragma omp parallel for reduction(+ : volume)
    ...
    double Volume(const TriangleMeshView& rTriangleMeshView, IndexType Dir)
    {
        ...
        rTriangleMeshView.VisitEachTriangle<WithNormals>([&](const auto& triangle) {
            const auto center = TriangleUtilities::Center(triangle);
            const double contribution = triangle.Normal[Dir] * center[Dir] * TriangleUtilities::Area(triangle);
            volume += contribution;
        });
    ...
    {
        if (rTriangleMeshView.NumOfTriangles() == 0) { return 0.0; }
    ...
    double EstimateQuality(const TriangleMeshView& rTriangleMeshView)
    {
        ...
        const double denom = numerical_guards::IsSafeDivisor(total_volume, MinimumQualityNormalizationDenominator)
                                 ? total_volume
                                 : MinimumQualityNormalizationDenominator;
        ...
        const double area_denominator =
            numerical_guards::IsSafeDivisor(total_area, MinimumQualityNormalizationDenominator)
                ? std::abs(total_area)
                : MinimumQualityNormalizationDenominator;
        const double error_area = Math::Norm(directional_areas) / area_denominator;
