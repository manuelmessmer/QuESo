...
std::array<double,5> QuESo::Compute(){
    ...
    const IndexType global_number_of_elements = mVoxelIndexer.NumberOfElements();
    ...
    for( int index = 0; index < static_cast<int>(global_number_of_elements); ++index) {
        ...
        if( status == IntersectionStatus::Inside || status == IntersectionStatus::Trimmed ) {
            ...
            const auto bounding_box_xyz = mVoxelIndexer.GetBoundingBoxXYZFromIndex(index);
            const auto bounding_box_uvw = mVoxelIndexer.GetBoundingBoxUVWFromIndex(index);
