...
class TestStrainEnergySteeringKnuckleKratos(QuESoTestCase):
    def run_test(self, filename, tolerance):
        ...
        volume = 0.0
        for element in pyqueso.elements("main"):
            for point in element.GetIntegrationPoints():
                volume += point.Weight()
        component_info = pyqueso.component_info("main")
        settings = pyqueso.settings("main")

        volume_info = component_info["quadrature_info"].GetDouble("represented_volume")
        self.assertAlmostEqual(volume, volume_info, places=7)

        ## Check component info
        # embedded_geometry_info
        self.assertEqual(component_info["embedded_geometry_info"].GetBool("is_closed"), True)
        self.assertAlmostEqual(component_info["embedded_geometry_info"].GetDouble("volume"), volume, places=5)
        ...
        # quadrature_info
        self.assertAlmostEqual(component_info["quadrature_info"].GetDouble("percentage_of_geometry_volume"), 100.0, places=5)
        self.assertEqual(component_info["quadrature_info"].GetInt("tot_num_points"), 13251)
        self.assertAlmostEqual(component_info["quadrature_info"].GetDouble("num_of_points_per_full_element"), 23.25, places=5)
        self.assertGreater(component_info["quadrature_info"].GetDouble("num_of_points_per_trimmed_element"), 26)
        self.assertLess(component_info["quadrature_info"].GetDouble("num_of_points_per_trimmed_element"), 27)
        ...
        self.assertEqual(component_info["background_grid_info"].GetInt("num_active_elements"), 498)
        self.assertEqual(component_info["background_grid_info"].GetInt("num_trimmed_elements"), 486)
        self.assertEqual(component_info["background_grid_info"].GetInt("num_full_elements"), 12)
        self.assertEqual(component_info["background_grid_info"].GetInt("num_inactive_elements"), 5752)
        ...
        # elapsed_time_info
        self.assertGreater(component_info["elapsed_time_info"].GetDouble("total"), 0.0)
        self.assertGreater(json_dict["elapsed_time_info"]["total"], 0.0)

        self.assertGreater(component_info["elapsed_time_info"]["volume_time_info"].GetDouble("total"), 0.0)
        self.assertGreater(component_info["elapsed_time_info"]["volume_time_info"].GetDouble("classification_of_elements"), 0.0)
        self.assertGreater(component_info["elapsed_time_info"]["volume_time_info"].GetDouble("computation_of_intersections"), 0.0)
        self.assertGreater(component_info["elapsed_time_info"]["volume_time_info"].GetDouble("solution_of_moment_fitting_eqs"), 0.0)
        self.assertGreater(component_info["elapsed_time_info"]["volume_time_info"].GetDouble("construction_of_ggq_rules"), 0.0)
        ...
        self.assertGreater(component_info["elapsed_time_info"]["conditions_time_info"].GetDouble("total"), 0.0)
        self.assertGreater(json_dict["elapsed_time_info"]["conditions_time_info"]["total"], 0.0)

        self.assertGreater(component_info["elapsed_time_info"]["write_files_time_info"].GetDouble("total"), 0.0)
        self.assertGreater(json_dict["elapsed_time_info"]["write_files_time_info"]["total"], 0.0)

        # conditions_infos_list
        conditions_info_list = component_info.GetList("conditions_infos_list")
