...
from pyqueso.kratos_interface.model_part_utilities import ModelPartUtilities
...
class TestBoundaryConditionsKratos(QuESoTestCase):
    ...
    def construct_b_spline_volume(self, model, QuESoSettings):
        ...
        tmp_parameters["lower_point_xyz"].SetVector(grid_settings.get_double_vector("lower_bound_xyz"))
        tmp_parameters.AddEmptyValue("upper_point_xyz")
        tmp_parameters["upper_point_xyz"].SetVector(grid_settings.get_double_vector("upper_bound_xyz"))

        tmp_parameters.AddEmptyValue("lower_point_uvw")
        tmp_parameters["lower_point_uvw"].SetVector(grid_settings.get_double_vector("lower_bound_uvw"))
        tmp_parameters.AddEmptyValue("upper_point_uvw")
        tmp_parameters["upper_point_uvw"].SetVector(grid_settings.get_double_vector("upper_bound_uvw"))

        tmp_parameters.AddEmptyValue("polynomial_order")
        tmp_parameters["polynomial_order"].SetVector(grid_settings.get_int_vector("polynomial_order"))
        tmp_parameters.AddEmptyValue("number_of_knot_spans")
        tmp_parameters["number_of_knot_spans"].SetVector(grid_settings.get_int_vector("number_of_elements"))

        self.run_modelers(model, modeler_settings)

    def test_penalty_support(self):
        queso_model = pyqueso.Model(json_filename="queso/tests/boundary_conditions_kratos/QuESoSettings_Penalty.json")
        queso_model.create()

        model = KM.Model()
        model_part = model.CreateModelPart("NurbsMesh")

        settings = queso_model.settings
        grid_settings = settings["background_grid_settings"]
        bounds_xyz = [grid_settings.get_double_vector("lower_bound_xyz"), grid_settings.get_double_vector("upper_bound_xyz")]
        bounds_uvw = [grid_settings.get_double_vector("lower_bound_uvw"), grid_settings.get_double_vector("upper_bound_uvw")]
        self.construct_b_spline_volume(model, settings)
        ModelPartUtilities.add_conditions_to_model_part(model_part, queso_model.conditions, bounds_xyz, bounds_uvw)
    ...
    def test_lagrange_support(self):
        queso_model = pyqueso.Model(json_filename="queso/tests/boundary_conditions_kratos/QuESoSettings_Lagrange.json")
        queso_model.create()
    ...
    def test_surface_load(self):
        queso_model = pyqueso.Model(json_filename="queso/tests/boundary_conditions_kratos/QuESoSettings_SurfaceLoad.json")
        queso_model.create()
    ...
    def test_pressure_load(self):
        queso_model = pyqueso.Model(json_filename="queso/tests/boundary_conditions_kratos/QuESoSettings_Pressure.json")
        queso_model.create()
