...
namespace Testing {
    ...
    BOOST_AUTO_TEST_CASE(TestDataSetCheckRequired)
    {
...

        {
            DataSetType data_set(
                DataSetType::KeySetInfoTypeTag<queso::key::detail::MainSettingsMainValuesTypeTagKeySetInfo>{}
            );
        ...
        {
            DataSetType data_set(
                DataSetType::KeySetInfoTypeTag<queso::key::detail::BackgroundGridSettingsMainValuesTypeTagKeySetInfo>{}
            );
    ...
    BOOST_AUTO_TEST_CASE(TestDataSetValue)
    {
        ...
        StringAccess::SetValue(
            data_set, "zero", Vector3i{ 1, 2, 3 }
        );  // The conversion from Vector3i to PointType is legal.
    ...
    BOOST_AUTO_TEST_CASE(TestDataSetGetValueKey)
    {
        ...
        QuESo_CHECK_POINT_NEAR(
            data_set.GetRequiredValue<PointType>(TestKeys3::zero), PointType({ 1.0, 2.0, 3.0 }), 1e-12
        );
    ...
    BOOST_AUTO_TEST_CASE(TestDataSetGetValueKeyName)
    {
        ...
        // Get values
        QuESo_CHECK_POINT_NEAR(
            StringAccess::GetValue<PointType>(data_set, "zero"), PointType({ 1.0, 2.0, 3.0 }), 1e-12
        );
