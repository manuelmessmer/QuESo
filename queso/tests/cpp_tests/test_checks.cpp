...
namespace Testing {
    ...
    BOOST_AUTO_TEST_CASE(AssertAcceptsNonCopyableExplicitBool)
    {
        if constexpr (!NOTDEBUG) {
            const auto p_value = std::make_unique<int>(1);
            QuESo_ASSERT(p_value, "A non-null unique pointer satisfies the assertion.");
        }
    }
