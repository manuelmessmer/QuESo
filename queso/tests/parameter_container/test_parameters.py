...
class TestParametersContainer(QuESoTestCase):
    def check_values(self, parameters):
        ...
        self.assertListsAlmostEqual(lower_bound_xyz, [-130.0, -110.0, -110.0], 10)
        lower_bound_uvw = parameters.LowerBoundUVW()
        self.assertListsAlmostEqual(lower_bound_uvw, [-130.0, -110.0, -110.0], 10)

        upper_bound_xyz = parameters.UpperBoundXYZ()
        self.assertListsAlmostEqual(upper_bound_xyz, [20.0, 190.0, 190.0], 10)
        upper_bound_uvw = parameters.UpperBoundUVW()
        self.assertListsAlmostEqual(upper_bound_uvw, [20.0, 190.0, 190.0], 10)

        polynomial_order = parameters.Order()
        self.assertListsAlmostEqual(polynomial_order, [2, 2, 2], 10)

        number_of_elements = parameters.NumberOfElements()
        self.assertListsAlmostEqual(number_of_elements, [5, 5, 5], 10)
