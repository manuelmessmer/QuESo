...
namespace Testing {
    ...
    using embedding::detail::AabbTree;
    using embedding::detail::AabbTriangleIntersector;

    BOOST_AUTO_TEST_SUITE(AabbTreeTestSuite)
    ...
    BOOST_AUTO_TEST_CASE(TouchingCubeTest1)
    {
        ...
        // Build aabb tree
        AabbTree tree(triangle_mesh.View());

        Vector3d lower_bound = { -2.0, -2, -2 };
        Vector3d upper_bound = { -1.5, 2, 2 };

        AabbTriangleIntersector aabb(lower_bound, upper_bound);
        auto results = tree.Query(lower_bound, upper_bound);

        std::vector<IndexType> intersected_triangles{};

        for (auto r : results) {
            const auto tri = triangle_mesh.Triangle<WithoutNormals>(r);
            // If tolerance>=0 intersection is not detected.
            const double tolerance_1 = 1e-8;
            QuESo_CHECK(!aabb.IntersectsTriangle(tri, tolerance_1));
            // If tolerance=0 intersection is detected.
            const double tolerance_2 = 0.0;
            QuESo_CHECK(aabb.IntersectsTriangle(tri, tolerance_2));
        }
    ...
    BOOST_AUTO_TEST_CASE(TouchingTriangleTest)
    {
        ...
        triangle_mesh.AddTriangle(
            { 0, 1, 2 },
            TriangleUtilities::Normal(
                TriangleProxy<WithoutNormals>{
                    triangle_mesh.Vertex(0), triangle_mesh.Vertex(1), triangle_mesh.Vertex(2) }
            )
        );
    ...
    BOOST_AUTO_TEST_CASE(CylinderFindIntersectedTrianglesTest)
    {
        ...
        std::ifstream myfile(base_dir + "/embedding/aabb_cylinder.txt");
            ...
            for (double yy = -1.5; yy <= 1.5; yy += 0.1) {
                for (double zz = -1; zz <= 11; zz += 0.1) {
                    ...
                    for (auto r : results) {
                        ...
                        if (aabb.IntersectsTriangle(tri, tolerance)) { intersected_triangles.push_back(r); }
                    }
    ...
    BOOST_AUTO_TEST_CASE(ElephantFindIntersectedTrianglesTest)
    {
        ...
        std::ifstream myfile(base_dir + "/embedding/aabb_elephant.txt");
    ...
    BOOST_AUTO_TEST_CASE(BunnyFindIntersectedTrianglesTest)
    {
        ...
        std::ifstream myfile(base_dir + "/embedding/aabb_bunny.txt");
