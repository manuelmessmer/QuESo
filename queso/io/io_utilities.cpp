...
//     // Read first 256 bytes (or less if file is smaller)
//     char buffer[256] = {};
//     file.read(buffer, sizeof(buffer));
//     std::streamsize bytesRead = file.gcount();

//     // Check if first non-whitespace characters are "solid"
//     std::string header(buffer, static_cast<size_t>(bytesRead));

//     // Trim leading whitespace
//     auto first_non_ws = header.find_first_not_of(" \t\r\n");
//     if (first_non_ws == std::string::npos) return false; // empty or whitespace only file

//     if (header.compare(first_non_ws, 5, "solid") != 0)
//         return false;

//     // Check if the buffer contains any non-printable characters (excluding whitespace)
//     for (std::streamsize i = 0; i < bytesRead; ++i) {
//         unsigned char c = static_cast<unsigned char>(buffer[i]);
//         if (c < 32 && c != 9 && c != 10 && c != 13) { // not whitespace or printable
//             return false; // binary data detected
//         }
//     }

//     // Optionally check for keywords "facet", "normal" etc in header (not mandatory)
//     // This can reduce false positives but is not foolproof.

//     return true; // likely ASCII STL
// }
