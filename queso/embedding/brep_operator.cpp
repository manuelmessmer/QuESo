...
Unique<StatusVectorType> BRepOperator::pGetElementClassifications(const MainDictionaryType& rSettings) const {
...
TrimmedDomainPtrType BRepOperator::pGetTrimmedDomain(PointView rLowerBound, PointView rUpperBound,
    ...
    while( iteration < 15){
        ...
        if( new_mesh.NumOfTriangles() > 0) {
            ...
            const double volume = MeshUtilities::Volume(mesh_view);
            bool volume_ratio = volume / volume_non_trimmed_domain > MinElementVolumeRatio;

            const double epsilon = MeshUtilities::EstimateQuality(mesh_view);
            if( epsilon < 1e-5  ){
                if( volume_ratio )
