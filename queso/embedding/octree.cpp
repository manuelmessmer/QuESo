    ...
    if (this->IsLeaf()) {
        ...
        if ((mLevel < MinLevel) || (mLevel < MaxLevel && mStatus == IntersectionState::trimmed)) {
            ...
            CreateNewNode(
                ...
                MaxLevel,
                0,
                MakeBox(r_lower_bound_xyz, (r_lower_bound_xyz + delta_xyz)),
                MakeBox(r_lower_bound_uvw, (r_lower_bound_uvw + delta_uvw)),
                pOperator
            );
            // Corner b (a+delta_x)
            CreateNewNode(
                ...
                MaxLevel,
                1,
                MakeBox((r_lower_bound_xyz + delta_x_xyz), ((r_lower_bound_xyz + delta_x_xyz) + delta_xyz)),
                MakeBox((r_lower_bound_uvw + delta_x_uvw), ((r_lower_bound_uvw + delta_x_uvw) + delta_uvw)),
                pOperator
            );
            // Corner c (g-delta_z)
            CreateNewNode(
                ...
                MaxLevel,
                2,
                MakeBox(((r_upper_bound_xyz - delta_z_xyz) - delta_xyz), (r_upper_bound_xyz - delta_z_xyz)),
                MakeBox(((r_upper_bound_uvw - delta_z_uvw) - delta_uvw), (r_upper_bound_uvw - delta_z_uvw)),
                pOperator
            );
            // Corner d (a+delta_y)
            CreateNewNode(
                ...
                MaxLevel,
                3,
                MakeBox((r_lower_bound_xyz + delta_y_xyz), ((r_lower_bound_xyz + delta_y_xyz) + delta_xyz)),
                MakeBox((r_lower_bound_uvw + delta_y_uvw), ((r_lower_bound_uvw + delta_y_uvw) + delta_uvw)),
                pOperator
            );
            // Corner e (a+delta_z)
            CreateNewNode(
                ...
                MaxLevel,
                4,
                MakeBox((r_lower_bound_xyz + delta_z_xyz), ((r_lower_bound_xyz + delta_z_xyz) + delta_xyz)),
                MakeBox((r_lower_bound_uvw + delta_z_uvw), ((r_lower_bound_uvw + delta_z_uvw) + delta_uvw)),
                pOperator
            );
            // Corner f (g-delta_y)
            CreateNewNode(
                ...
                MaxLevel,
                5,
                MakeBox(((r_upper_bound_xyz - delta_y_xyz) - delta_xyz), (r_upper_bound_xyz - delta_y_xyz)),
                MakeBox(((r_upper_bound_uvw - delta_y_uvw) - delta_uvw), (r_upper_bound_uvw - delta_y_uvw)),
                pOperator
            );
            // Corner g
            CreateNewNode(
                ...
                MaxLevel,
                6,
                MakeBox((r_upper_bound_xyz - delta_xyz), r_upper_bound_xyz),
                MakeBox((r_upper_bound_uvw - delta_uvw), r_upper_bound_uvw),
                pOperator
            );
            // Corner h (g-delta_x)
            CreateNewNode(
                ...
                MaxLevel,
                7,
                MakeBox(((r_upper_bound_xyz - delta_x_xyz) - delta_xyz), (r_upper_bound_xyz - delta_x_xyz)),
                MakeBox(((r_upper_bound_uvw - delta_x_uvw) - delta_uvw), (r_upper_bound_uvw - delta_x_uvw)),
                pOperator
            );
        }
    } else {
        for (IndexType i = 0; i < 8UL; ++i) {
            if (mChildren[i]) { mChildren[i]->Refine(MinLevel, MaxLevel, pOperator); }
        }
    }
}
...
template<typename TOperator>
void Octree<TOperator>::Node::CreateNewNode(
    IndexType MinLevel,
    IndexType MaxLevel,
    IndexType ChildIndex,
    const BoundingBoxType& rBoundsXYZ,
    const BoundingBoxType& rBoundsUVW,
    const TOperator* pOperator
)
{
    const auto status = pOperator->template GetIntersectionState<CoordinateSpace::global>(
        rBoundsXYZ.lower, rBoundsXYZ.upper, AabbIntersectionPolicy::SnapEroded
    );
